...
/*For SD card & RTC */
  ...
  myFile = SD.open("Clock.txt",O_CREAT | O_APPEND | O_WRITE); // File created
...
void loop() {
  DateTime now = rtc.now();
  Serial.print("<-After some Seconds->\t"); delay(250);Serial.println(now.timestamp(DateTime::TIMESTAMP_TIME));
  ...
  if (pushSwitchState == LOW){
    ...
    // Open file 
    myFile = SD.open("Clock.txt",O_CREAT | O_APPEND | O_WRITE);
    // Write sesnor data on SD card
    myFile.print("RTC and SD card write test @ ");
    myFile.println(now.timestamp(DateTime::TIMESTAMP_TIME));
    //myFile.print("->\t");
    //myFile.print(String("Air profile:")+air_profile);
