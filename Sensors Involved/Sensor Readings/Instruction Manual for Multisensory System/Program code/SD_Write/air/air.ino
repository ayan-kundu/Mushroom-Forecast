...
// Request frame for the RH sensor
...
void setup()
{
  ...
  myFile = SD.open("Air.txt",O_CREAT | O_APPEND | O_WRITE); // File created
  if (myFile) {
...
void loop()
{
  ...
  Serial.println("Waited for air response");
  ...
  if (pushSwitchState == LOW){
    ...
    // Open file 
    myFile = SD.open("Air.txt",O_CREAT | O_APPEND | O_WRITE);
    // Write sesnor data on SD card
    myFile.println("");
    myFile.print(noww.timestamp(DateTime::TIMESTAMP_TIME));
    myFile.print("->\t");
    myFile.print(String("Air humidity:")+Moisture_Percent+String("%"));myFile.println(String("  |  Air temperature:")+Temperature_Celsius+String("°C"));
    ...
    Serial.print(String("->Air humidity:")+Moisture_Percent+String("%"));Serial.println(String("  |  Air temperature:")+Temperature_Celsius+String("°C"));
    Serial.println();
    myFile.close();
