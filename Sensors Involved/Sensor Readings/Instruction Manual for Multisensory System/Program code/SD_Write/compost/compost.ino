...
void setup()
{
  ...
  myFile = SD.open("Compost.txt",O_CREAT | O_APPEND | O_WRITE); // File created
  if (myFile) {
...
void loop()
{
  ...
  Serial.println("Waited for Soil response");
  ...
  if (pushSwitchState == LOW){
    ...
    // Open file 
    myFile = SD.open("Compost.txt",O_CREAT | O_APPEND | O_WRITE);
