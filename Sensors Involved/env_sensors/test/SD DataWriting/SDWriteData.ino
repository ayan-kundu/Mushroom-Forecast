...
void loop()
{
  ...
  if (pushSwitchState == LOW){
    ...
    myFile.print("->\t");
    myFile.print(String("Soil moisture:")+Moisture_Percent_global+String("%"));myFile.println(String("  Soil temperature:")+Temperature_Celsius_global+String("°C"));
    // Display that data is written down the SD
    Serial.println("Push Switch pressed, file write triggered");
    Serial.print(String("-> Soil moisture:")+Moisture_Percent_global+String("%"));Serial.println(String("   Soil temperature:")+Temperature_Celsius_global+String("°C"));
    myFile.close();
