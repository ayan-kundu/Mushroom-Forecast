...

{
  ...
  String soil_profile=soilSensor();
  //soil_moisture=moisture_percent;
  //delay(1100);
  Co2_value=CO2();

  Serial.println(String("->Co2: ")+Co2_value+String("ppm | ")+soil_profile);
...
String soilSensor(){//float *temp
  ...
  if (modbus.available() >= 9) // If valid response received
  {
    ...
    int Moisture_Int = int(soilSensorResponse[5] << 8 | soilSensorResponse[6]); // code worked with this 
    ...
    int Temperature_Int = int(soilSensorResponse[3] << 8 | soilSensorResponse[4]); // code worked with this 
  ...
  String soil_profile;
  soil_profile=(String("Soil Moisture: ")+String(Moisture_Percent) + String("%  ")+ String("Soil temperature: ")+String(Temperature_Celsius)+String("°C | "));
  return soil_profile;
